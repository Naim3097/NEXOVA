  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const products: ParsedProduct[] = jsonData.map((row: any) => {
        ...
        const code = row['Product Code'] || row['Code'] || row['SKU'] || row['code'] || row['sku'] || '';
        ...
        const image_url = row['Image URL'] || row['Image'] || row['image_url'] || row['image'] || '';
        const stock = parseFloat(row['Stock'] || row['Quantity'] || row['stock'] || '0') || 0;

        // Handle price - try multiple column names
        let base_price = row['Base Price'] || row['Price'] || row['Sale Price'] || row['Sale Price (MYR)'] || row['base_price'] || row['price'] || 0;
        if (typeof base_price === 'string') {
          base_price = parseFloat(base_price.replace(/[^0-9.-]/g, '')) || 0;
        }

        const currency = row['Currency'] || row['currency'] || 'RM';
        const notes = row['Notes'] || row['notes'] || row['Sizes/Variants'] || '';
        const status = (row['Status'] || row['status'] || 'active').toLowerCase() === 'inactive' ? 'inactive' : 'active';
      ...
      if (products.length === 0) {
        setParseError('No valid products found. Make sure your file has columns: Code/SKU, Name, and Price');
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-blue-900 font-medium">Need a template?</p>
                    ...
                    <tbody className="divide-y">
                      {parsedProducts.slice(0, 10).map((product, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          <td className="px-3 py-2 text-gray-900">{product.code}</td>
                          <td className="px-3 py-2 text-gray-900 truncate max-w-[200px]">{product.name}</td>
                          <td className="px-3 py-2 text-right text-gray-900">
                            {product.currency} {product.base_price.toFixed(2)}
                          </td>
                          <td className="px-3 py-2 text-right text-gray-900">{product.stock}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {/* Upload Result */}
          {uploadResult && (
            <div className={`mb-6 p-4 rounded-lg ${uploadResult.success ? 'bg-green-50' : 'bg-red-50'}`}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {uploadResult.success && uploadResult.skipped && uploadResult.skipped > 0 && (
                    <p className="text-sm text-green-700 mt-1">
                      {uploadResult.skipped} product(s) were skipped due to validation errors.
                    </p>
                  )}
            ...
            >
              {uploading ? (
                ...
                <>
                  ...
                  Upload {parsedProducts.length > 0 ? `(${parsedProducts.length})` : ''}
                </>
              )}
            </Button>
          )}
