...
export const PublishDialog = ({
  ...
  const [urlType, setUrlType] = useState<'path' | 'subdomain' | 'custom'>(
    'path'
  );
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          {!publishedUrl ? (
            ...
            <>
              <div
                className={`${isPro ? 'bg-purple-50 border-purple-200' : 'bg-[#5FC7CD]/10 border-[#5FC7CD]/20'} border rounded-xl p-4`}
              >
                <h4
                  className={`font-semibold mb-2 ${isPro ? 'text-purple-900' : 'text-[#455263]'}`}
                >
                  Ready to publish "{projectName}"?
                </h4>
                <p
                  className={`text-sm mb-2 ${isPro ? 'text-purple-700' : 'text-[#455263]'}`}
                >
                  Your landing page will be generated and made publicly
                  accessible. You can unpublish or update it anytime.
                </p>

                {userSubdomain ? (
                  <div
                    className={`mt-3 pt-3 border-t ${isPro ? 'border-purple-200' : 'border-[#E2E8F0]'}`}
                  >
                    <p
                      className={`text-sm font-medium mb-1 ${isPro ? 'text-purple-900' : 'text-[#455263]'}`}
                    >
                    ...
                    >
                      ...
                      <span className="font-mono font-semibold">
                        {userSubdomain}.yourdomain.com
                      </span>
                    </p>
                    {!isPro && (
                      <p className="text-xs text-[#5FC7CD]">
                        💡 Upgrade to Pro for custom domains like{' '}
                        <span className="font-mono">www.yourdomain.com</span>
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="mt-3 pt-3 border-t border-[#E2E8F0]">
                    <p className="text-sm font-medium text-[#455263] mb-1">
                      📍 Publishing URL:
                    </p>
            ...
            <>
              ...
              <div>
                <label className="text-sm font-medium text-[#455263] mb-2 block flex items-center justify-between">
                  <span>Published URL</span>
                  {urlType === 'subdomain' && (
                    <span className="text-xs bg-[#5FC7CD]/10 text-[#5FC7CD] px-2 py-1 rounded-full font-semibold">
                      SUBDOMAIN
                    </span>
                  )}
                  {urlType === 'custom' && (
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={publishedUrl}
                    readOnly
                    className="flex-1 px-3 py-2 border border-[#E2E8F0] rounded-xl bg-[#F8FAFC] text-sm font-mono focus:ring-[#5FC7CD] focus:border-[#5FC7CD]"
                  />
                  <Button
                    variant="outline"
                    size="sm"
              ...
              <div className="flex gap-2">
                <Button onClick={handleVisit} variant="teal" className="flex-1">
